...
data.forEach(item =>{
    ...
    if(item.category  === "Samsung")
    add_Cart.innerHTML += `
    <!-- Cart item -->
                        <div id="${item.id}" class="Cart col-md-2  text-white mt-3  " >
                            <div class="data-cart  d-flex align-content-between flex-column" style="background-color: rgb(196, 139, 69);">
                                <div class="img-data-cart w-80  h-80 mt-3" style="height: 250px;">
                                    <img class="w-100 h-100" src="${item.image}" alt="">
                                </div>
                                <div class="title-data-cart py-1 text-dark font-weight-bold">${item.name}</div>
